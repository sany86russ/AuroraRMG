...
### 🧭 «Охрана границ» теперь видна и в расширенном режиме (по фидбэку игрока)

В 1.3.3 настройка **«Охрана границ»** появилась в простом режиме, но в расширенном тот же параметр назывался иначе — слайдер «Сила границ/порталов» — и его не находили. Исправлено:

- ползунок переименован в **«Охрана границ/порталов»** (вкладка **«Настройка зон» → блок «◆ НАСТРОЙКА ЗОН»**), чтобы искался по тому же слову, что и в простом режиме;
- добавлена всплывающая подсказка: это та же настройка, что «Охрана границ» в простом режиме (25–300 %, по умолчанию 100 %; выше — крепче «фейс-контроль»).

Соответствие уровней простого режима ползунку: **Слабая** ≈ 45–80 %, **Обычная** ≈ 80–140 %, **Сильная** ≈ 150–220 %, **Крепость** ≈ 230–300 %.

### 🛡 Сила охраны на границах зон *(из 1.3.3)*

![Простой режим](docs/ui-simple.png)

В **простом режиме** появилась настройка **«Охрана границ»** — теперь можно задать, насколько сильны монстры-стражи на проходах между зонами:

- **Слабая** — проходы пробиваются легко (ранние рывки проходят);
- **Обычная** — как раньше (карты по старым сидам не меняются);
- **Сильная** — крепкие ворота;
- **Крепость** — настоящая стена: агрессивный соперник (в т.ч. поумневший после патча ИИ) надолго заперт в своей зоне и не прорвётся к вам ранним рашем — тот самый «фейс-контроль».

Раньше сила границ задавалась только в расширенном режиме (слайдер «Сила границ/порталов») — теперь это под рукой и в простом. Фактическая сила (в %) показывается в сводке после генерации.

### ⚙️ Качество быстрой генерации *(из 1.3.2)*

- **Шахты у каждого игрока.** Быстрые карты теперь гарантированно дают стартовые шахты (дерево/руда/золото), а не только разовые склады ресурсов — у игрока есть нормальная экономика.
- **Стартовый террейн под фракцию.** Дом каждого игрока теперь на родном террейне его фракции — даже на тематических картах (нейтральные зоны остаются разнообразными).
- **Перенос Простой → Расширенный.** При «Открыть в расширенном режиме» теперь корректно переносится число нейтральных зон (раньше показывало 0).

### 🗺 Ручной контроль в редакторе зон *(из 1.3.2)*

В **визуальном редакторе** («🗺 Редактор») можно вручную переопределять то, что генератор делает автоматически:
...
- **Связи, порталы и охрана — точечно.** Какие зоны соединять (например, параллельные коридоры bronze→silver→gold), какие — через **порталы**, и **сила охраны на каждое соединение** (как на «Highway»). Подробнее — в разделе [Визуальный редактор зон](#-визуальный-редактор-зон).

### 🎲 Простой режим стал чище *(из 1.3.1)*

- **Превью шаблона скрыто по умолчанию.** Простой режим теперь ведёт результатом («что получилось» + оценка баланса), а структуру карты можно раскрыть кнопкой **«👁 Показать превью шаблона»** — так генерация ощущается по-настоящему случайной.
- **«Сбросить настройки» работает и в простом режиме.** Раньше кнопка сбрасывала только расширенный режим — теперь и простой тоже (свежий сид, чистый результат).

### 🗺 Большие карты — больше 240×240 *(из 1.3.0)*

По просьбам игроков теперь можно создавать карты крупнее прежнего предела 240×240 — на них интереснее долгие, насыщенные партии.

- В **простом режиме** — новый размер карты **«Огромная (256–400)»**.
- В **расширенном режиме** опция больших карт **256×256 … 512×512** теперь на виду — заметная отметка прямо под выбором размера (она была и раньше, но её было трудно найти).
- Официальные шаблоны заканчиваются на 240×240, но движок игры большие карты поддерживает.

### ⚖ Анализ карты — баланс и состав

После генерации (в **обоих** режимах) сразу видно, **что получилось** и **насколько это честно**:

- **Оценка баланса 0–100** — насколько равны старты игроков (богатство, простор для развития, дистанции, доступ к замкам), плюс понятные замечания: бедный старт, неравный доступ к замкам, слишком близкие старты.
- **«Что внутри карты»** — зоны по ролям (игроки / нейтральные / замки), число связей, суммарные сокровища и ресурсы.

### 🏛 Курируемые ориентиры

В нейтральных зонах теперь гарантированно появляются **фирменные объекты** (драконьи утопии, лаборатории, обсерватории и т.п.) — карта ощущается задизайненной, а не просто случайной.

---

## 📑 Содержание

- [Что нового в 1.3.4](#-что-нового-в-134)
...
| 🧩 **Полный контроль** | топология зон, экономика, сила монстров, ландшафт, вода, условия победы, размер карты **до 512×512** |
...
3. Нажми **«⚔ Создать карту»** — справа появится превью и краткая сводка.
4. Нажми **«💾 Сохранить в папку игры»** — введи название, и карта сразу окажется в игре.
5. Запусти Olden Era → создай новую игру → выбери свой шаблон в списке.
...
2. На вкладке **«Правила карты»** в блоке **ШАБЛОН** открой список **«Пресет»** и выбери заготовку (например, *«1v1 Классика — Стандарт»*).
...
<img src="https://raw.githubusercontent.com/sany86russ/AuroraRMG/main/docs/ui-simple.png" alt="Простой режим" width="100%"/>

Для тех, кто **не хочет разбираться в шаблонах**. Вы задаёте лишь несколько понятных опций — остальное генератор подбирает случайно в разумных пределах и сразу выдаёт готовую играбельную карту:

| Опция | Что задаёт |
|---|---|
| **Игроков** | Сколько игроков (стартовых замков) на карте. |
| **Тип игры** | Расстановка: **Дуэль (1×1)**, **Каждый сам за себя**, **Против монстров (PvE)**, **Командная игра** *(этого режима пока нет в самой игре)*. |
| **Размер карты** | Маленькая / Средняя / Большая / **Огромная (256–400)**. «Огромная» выходит за официальный предел 240×240 (эксперим.), но движок игры её поддерживает. |
...
| **Условие победы** | Все реальные режимы игры — классика, удержание города/столицы, решающая битва, турнир (см. [Условия победы](#-условия-победы-и-спец-режимы)). |
| **Дополнительно** | Вода, порталы, усиленные нейтралы. |
| **Сид** 🎲 📋 | «Отпечаток» карты: один и тот же сид всегда даёт одну и ту же карту. **Делитесь сидом**, чтобы сыграть с друзьями на идентичной карте. 🎲 — новый случайный, 📋 — скопировать. |
...
<img src="https://raw.githubusercontent.com/sany86russ/AuroraRMG/main/docs/ui-rules.png" alt="Расширенный режим — вкладка «Правила карты»" width="100%"/>

Классический режим со **всеми** настройками на четырёх вкладках (**Правила карты · Настройка зон · Наполнение зон · Бонусы и баны**) и правой панелью генерации/превью. Здесь доступно всё: топология, экономика, сила монстров, ландшафт, вода, условия победы, баны, тонкая раскладка нейтральных зон, обязательный контент и **визуальный редактор зон**. Подробно — ниже в разделах [Обзор интерфейса](#-обзор-интерфейса) и [Все настройки](#️-все-настройки-подробно).
...
| 🗺 **Большие карты: 256×256 … 512×512** `[ЭКСП.]` | Заметная отметка прямо под выбором размера — добавляет в список крупные размеры (до 512×512). Официальных шаблонов такого размера нет, поэтому они не гарантированы, но движок игры их поддерживает (игроки водят партии на 300–400). В **Простом режиме** этому соответствует размер **«Огромная»**. |
...
> Симметрия в формате `.rmg.json` не задаётся отдельным полем — она возникает из топологии. Поэтому баланс мы **измеряем**, а не навязываем. Для максимально честной игры 1v1 выбирай топологию **Сбалансированная** или режим **Турнир**.
...
- **Правка:** перетаскивайте зоны; **инспектор** справа правит выбранную зону (имя, размер, раскладка, дипломатия, охрана, **замок/аванпост**) или связь (от/к, **тип**, **сила охраны**, дорога).
...
- **Фирменные ориентиры.** В нейтральных зонах генератор гарантированно ставит по одному заметному объекту согласно уровню зоны (драконьи утопии, лаборатории, обсерватории и т.п.) — чтобы карта ощущалась задизайненной, а не просто случайной.
...
> - **Большие карты** (256×256 … 512×512): официальных шаблонов такого размера нет; движок игры их поддерживает (игроки водят партии на 300–400), но поведение на самых крупных (ближе к 512) не гарантировано.
...
**Простой** — выбираешь несколько базовых опций и жмёшь «Создать карту», всё остальное подбирается автоматически (идеально для быстрой игры). **Расширенный** — полный ручной контроль над всеми параметрами на четырёх вкладках. Переключатель — в шапке, рядом с языком; по умолчанию открывается Простой. Карту из простого режима можно «Открыть в расширенном» и доработать. Подробнее — [Режимы работы](#-режимы-работы-простой-и-расширенный).

**Что такое «сид» в Простом режиме?**
Это «отпечаток» карты. Один и тот же сид всегда даёт одну и ту же карту, поэтому им удобно делиться: друг вводит тот же сид — и играет на идентичной карте. Кнопка 🎲 — новый случайный сид, 📋 — скопировать текущий.

**Можно ли создавать карты больше 240×240?**
Да. В **Простом режиме** выбери размер **«Огромная (256–400)»**; в **Расширенном** поставь отметку **«🗺 Большие карты: 256×256 … 512×512»** прямо под выбором размера — в списке появятся крупные размеры (до 512×512). Официальные шаблоны заканчиваются на 240×240, но движок игры большие карты поддерживает (поэтому опция помечена экспериментальной).
