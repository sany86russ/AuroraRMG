...
Главное нововведение — **простой режим генерации** для тех, кто не хочет разбираться в шаблонах. Выбираете несколько понятных опций — **число игроков, тип игры, размер карты, длительность, уровень хаоса и условие победы** — и жмёте **«Создать карту»**. Всё остальное подбирается случайно в разумных пределах, и вы получаете готовую играбельную карту.

![Простой режим](docs/ui-simple.png)

- Переключатель **Простой / Расширенный** в верхней панели (рядом с языком). Полный ручной режим со всеми настройками никуда не делся.
- **Сид карты** — её «отпечаток»: один и тот же сид всегда даёт одну и ту же карту. Делитесь сидом, чтобы сыграть с друзьями на идентичной карте (🎲 — новый случайный, 📋 — скопировать).
- **Сохранение прямо в папку игры** одной кнопкой (с запросом названия) — карта сразу появится в списке «Своей игры».
- Все реальные **условия победы** игры: классика, удержание города, удержание столицы, решающая битва, турнир — у каждого свои правила.
- Краткая **подсказка по каждой настройке** и оценка длительности партии прямо в окне.
- Приложение **запоминает** ваши последние выборы.

### 🛠 Прочее

- **Исправлена загрузка официальных шаблонов игры.** Редактор не открывал часть штатных шаблонов (например `Expanse.rmg.json`) из-за того, что игра пишет некоторые поля то строкой, то массивом — теперь читаются все 120.

---

## 📑 Содержание

- [Что нового в 1.2.0](#-что-нового-в-120)
...
Дальше: **«⚔ Создать карту»** → справа появятся **превью** и краткая **сводка** (включая оценку длительности партии) → **«💾 Сохранить в папку игры»** (спросит название и положит карту прямо в игру) либо **«Сохранить как…»**. Внизу окна — **подсказка по каждой настройке**. Любую карту можно **«Открыть в расширенном режиме»** и доработать вручную.
