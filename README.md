...
[📥 Скачать](#-установка) · [🚀 Быстрый старт](#-быстрый-старт) · [🧭 Обзор интерфейса](#-обзор-интерфейса) · [⚙️ Все настройки](#️-все-настройки-подробно) · [🎲 Пресеты](#-готовые-пресеты-37) · [❓ FAQ](#-частые-вопросы-faq)
...
> Идея и базовый подход к генерации `.rmg.json` взяты у него — спасибо автору оригинала! AuroraRMG — это переработанная версия с новым оформлением (тема **Aurora**), полной русской локализацией, расширенным набором настроек и собственным автообновлением. Подробнее — в разделе [Благодарности](#-благодарности-и-происхождение).
...
- [Готовые пресеты (37)](#-готовые-пресеты-37)
...
| | |
|---|---|
| 🎲 **37 готовых пресетов** | 1v1 Классика, 1v1 Один герой, FFA на 3–8 игроков, спец-режимы — загрузка в один клик |
...
| 🌑 **Тема Aurora** | тёмный неоновый интерфейс (фиолетовый → циан), полностью на русском |
...
> [!TIP]
> При первом запуске Windows SmartScreen может предупредить о «неизвестном издателе» (файл не подписан сертификатом). Нажми **«Подробнее» → «Выполнить в любом случае»**. Исходный размер файла — около 65 МБ.
...
> 💡 Скриншоты сделаны на актуальной версии в теме **Aurora**. Интерфейс полностью на русском.
...
Все пресеты заточены прежде всего под честную игру 1v1, но есть и FFA, и спец-режимы. Каждый протестирован: автотест проверяет, что сгенерированная карта реально соответствует описанию.
...
- 🌑 новое оформление — тёмная неоновая тема **Aurora** и новая иконка/логотип;
- 🇷🇺 полная **русская локализация** интерфейса;
- 🎲 набор из **37 готовых пресетов** с автотестами соответствия;
- ⚙️ расширенные параметры окружения (ландшафт, агрессия, вода, дипломатия, озёра, лазейки) и тонкая настройка зон;
- 🔄 собственная система **автообновления** через релизы этого репозитория.
